...
HRESULT CaptureFrame() {
    ...
    hr = CoCreateInstance(CLSID_FilterGraph, NULL,
        CLSCTX_INPROC, IID_PPV_ARGS(&pGraph));
    if (FAILED(hr)) {
        goto release_zone;
    }
    ...
    hr = pGraph->AddFilter(pGrabberF, L"Frame grabber");
    if (FAILED(hr)) {
        goto release_zone;
    }
    ...
    hr = pGraph->AddFilter(pSourceF, gcap.wachFriendlyName);
    if (FAILED(hr)) {
        goto release_zone;
    }
    ...
    long evCode;
    hr = pEvent->WaitForCompletion(10000, &evCode);
    ...
    if ((mt.formattype == FORMAT_VideoInfo) &&
        ...
        hr = WriteBitmap(L"screenShot.bmp", &pVih->bmiHeader,
...
LONG PASCAL AppCommand(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(id)
    {
        ...
        case MENU_START_CAP:
            ...
            BuildCaptureGraph();
            if (!StartCapture()) { //my fix
                BuildPreviewGraph();
                StartPreview();
            }
            break;
            
        case ID_CAPTURE_CAPTUREFRAME:
            CaptureFrame();
            //GetPreviewShot();
            break;
